...
for (i in 1:nrow(data_frame)){
  data_frame$pos_tagged[i] <- tagPOS(data_frame$coref_post_message[i])$POStagged
}
...
for (i in 1:nrow(data_frame)){
  annotations <- annotate(data_frame$coref_post_message[i], pipeline)
  doc <- AnnotatedPlainTextDocument(data_frame$coref_post_message[i], annotations)
