...
directory <- "/Users/Benedikt/Documents/researchProj2/ABSA-TensorFlow/"
dir.create(directory, recursive = TRUE, showWarnings = FALSE)
setwd(directory)

json_data <- rjson::fromJSON(file= "stanford-corenlp-python/posts_co-referenced_lower.json")
...
for (i in 1:nrow(data_frame)){
  data_frame$pos_tagged[i] <- tagPOS(data_frame$coref_post_message_lowered[i])$POStagged
}



sink("tagging/pos_tagged_coref.json")
...
for (i in 1:nrow(data_frame)){
  annotations <- annotate(data_frame$coref_post_message_lowered[i], pipeline)
  doc <- AnnotatedPlainTextDocument(data_frame$coref_post_message_lowered[i], annotations)
...
sink("tagging/complete_tagged_coref.json")
cat(jsonlite::toJSON(data_frame, pretty = TRUE))
sink()
