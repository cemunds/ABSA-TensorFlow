...
\begin{document}
	...
	In the following section, our approach towards the aspect based sentiment analysis will be presented. To only look at relevant data, our methods of pre-processing will be discussed first. Then, we will show how the products and aspects are extracted. Afterwards, the NLTK module \textit{VADER} is used to determine the sentiment polarities that are present in the specific Facebook posts.
		...
		Our sentiment analysis has to be applied for every comment on the Facebook page of Tesco. Therefore, a model which deals with the problem of having social media texts is largely favored. A tool for lexicon and rule-based sentiment analysis of social media text is VADER, which is the abbreviation for \textit{Valence Aware Dictionary and sEntiment Reasoner} \cite{vader}. This model is included in the Python library NLTK and enables us to use a lexicon and rule-based sentiment analysis tool that has been developed to specifically understand the sentiments expressed in social media channels, e.g. by understanding emoticons as well as slang words.
		
				
		To apply VADER model, the tagged posts as well as their entities and aspects are used. Every posts is parsed into the VADER model and the polarity of each post is calculated. To determine the polarity of each entity, we look whether the entity appears in a sentence of the post. If it does, the general polarity of that sentence will be calculated. To calculate the polarity towards the entity, the following formula is applied:
		
		If the positive polarity is bigger and the compound polarity is greater than 0.3, this sentence gets the polarity \textit{positive}. The threshold of 0.3 for the compound polarity was chosen because only checking whether the polarity towards positive is the highest is not enough to be accurate. If for example the polarity towards positive is 0.5001 and the polarity towards negative 0.4999, this would not be a representative positive sentence. For this example the compound value would be low which is the reason the 0.3 threshold is included in our formula. This ensures that only posts which have a reasonable difference between the positive and negative polarity are labeled as \textit{positive}. 
		
		Likewise, if the negative polarity is bigger and the compound polarity is smaller than -0.3, this sentence gets the polarity \textit{negative}. The threshold of -0.3 was chosen for the same reasons as above, with the only difference being that now a very low compound value is wanted to ensure that the negative is bigger than the positive polarity. 
		...
		After applying this formula we look whether an aspect appears in that particular sentence. If it does, the collection of entity, aspect and polarity gets added to the sentiment list. If it does not, the entity and polarity get added to the sentiment list under the aspect \textit{general}.
		
		The evaluated posts then create a new list containing the name of an entity and their positive, negative and neutral score by summing the occurrences of a positive, negative or neutral polarity.
...
\end{figure}		
		...
		Such results can be found for many products and aspects, e.g delivering insights into which Tesco stores receive mostly good or bad feedback if the threshold for the occurrence of aspects is lowered from 10\%.
